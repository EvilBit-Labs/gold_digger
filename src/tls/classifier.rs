    ...
    pub fn from_rustls_error(error: rustls::Error, hostname: Option<&str>) -> Self {
        match error {
            rustls::Error::InvalidCertificate(cert_error) => match cert_error {
                ...
                // `CertificateError` is `#[non_exhaustive]`; future rustls
                // releases may add new variants. Fall back through the
                // typed `Display` rather than `{:?}` so users see a
                // friendly message (todo #053) and the variant still
                // routes to a certificate-specific exit code (todo #060).
            ...
            rustls::Error::PeerIncompatible(incompatible_error) => {
                // `rustls::PeerIncompatible` does not implement `Display`
                // in rustls 0.23, only `Debug`. We emit the Debug form so
                // the routing keyword (`tls`/`version`/`cipher`) is still
                // discoverable; the variant name is friendly enough on
                // its own (e.g. `Tls12NotOffered`). If rustls grows a
                // Display impl this should be revisited (todo #053).
                let error_debug = format!("{:?}", incompatible_error);
                let debug_lower = error_debug.to_lowercase();
                if debug_lower.contains("tls") || debug_lower.contains("version") {
                    Self::ProtocolVersionMismatch {
                        message: format!("TLS version incompatibility: {}", error_debug),
                    }
                } else {
                    Self::CipherSuiteNegotiationFailed {
                        message: format!("Cipher suite negotiation failed: {}", error_debug),
                    }
                }
            },
            rustls::Error::PeerMisbehaved(misbehavior) => Self::PeerMisbehaved {
                // `rustls::PeerMisbehaved` does not implement `Display`
                // in rustls 0.23. Variant names are descriptive enough
                // (e.g. `KeyEpochWithPendingFragment`) that the Debug
                // form is operator-friendly.
                message: format!("Server violated TLS protocol: {:?}", misbehavior),
            },
            rustls::Error::AlertReceived(alert) => Self::ServerAlert {
                // `AlertDescription` is generated by a rustls macro and
                // does not implement `Display`. Debug yields the alert
                // name (`HandshakeFailure`, `BadCertificate`, ...) which
            ...
            // `rustls::Error` is `#[non_exhaustive]`; new releases may
            // add variants we haven't seen. Per todo #060, surface the
            // top-level `rustls::Error` Display text so operators get
            // the variant detail rather than a misleading generic
            // "handshake failed" with no context. If a future variant
            // warrants its own typed `TlsError` arm we add it above
            // this fallback.
            _ => Self::HandshakeFailed {
                message: format!("TLS error (unhandled rustls variant): {}", error),
            },
        }
    }
}
