...
fn benchmark_json_write(c: &mut Criterion) {
    ...
    group.bench_with_input(
        ...
        |b, data| {
            b.iter(|| {
                let mut output = Cursor::new(Vec::new());
                json::write(black_box(data.clone()), &mut output).unwrap();
                black_box(output.into_inner())
            })
        },
    );
    ...
    group.bench_with_input(
        ...
        |b, data| {
            b.iter(|| {
                let mut output = Cursor::new(Vec::new());
                json::write_with_pretty(black_box(data.clone()), &mut output, true).unwrap();
                black_box(output.into_inner())
            })
        },
    );
    ...
    // Null-heavy dataset
    let null_data = create_test_data_with_nulls(100, 10);
    group.throughput(Throughput::Elements(null_data.len() as u64));
    group.bench_with_input(
