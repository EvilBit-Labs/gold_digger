...
- **`src/run.rs`** — `pool.get_conn()` failures route through `classify_mysql_pool_error` for the same reasons. This was originally a `anyhow!("Database connection failed: {}", e)` site that bypassed both the typed classifier AND credential redaction. `map_query_error` likewise returns typed `GoldDiggerError::Query` / `DbAuth` (the connection-class `CR_*` codes routed via `is_connection_class_error`) instead of an untyped `anyhow!`, with unit tests asserting exit codes for a connection-named identifier and a leak-free access-denied message.
