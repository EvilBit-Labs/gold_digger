...
- **`src/run.rs`** — `pool.get_conn()` failures route through `classify_mysql_pool_error` for the same reasons. This was originally a `anyhow!("Database connection failed: {}", e)` site that bypassed both the typed classifier AND credential redaction.
- **Commits**: `1ea0c3d` (foundation: typed enum + downcast classifier + preservation in `connection.rs`), `5d6458e` (`pool.get_conn()` typed routing).
